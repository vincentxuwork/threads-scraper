...
class SQLiteCursorWrapper:
    """游標包裝器，將 PostgreSQL 風格的 %s 佔位符轉換為 SQLite 的 ?"""
...
class ThreadsDatabase:
    ...
    def get_connection(self):
        """取得資料庫連線（與 PostgreSQL 版本保持一致的介面）"""
        conn = sqlite3.connect(self.db_path)
        conn.row_factory = sqlite3.Row  # 支援字典式訪問
        return SQLiteConnectionWrapper(conn)  # 包裝連接以支持 %s 佔位符
